...
def main():
    cap = cv2.VideoCapture('store1.mp4')
    ...
    for i in range(0, num_frames):
        ...
        if (first_iteration_indicator == 1):
            ...
            k = cv2.applyColorMap(accum_image, cv2.COLORMAP_PINK)
            r = cv2.addWeighted(frame, 0.7, k, 0.7, 0)
            cv2.imshow('only', k)
            cv2.imshow('frame',r)
    ...
    color_image = im_color = cv2.applyColorMap(accum_image, cv2.COLORMAP_SUMMER)
